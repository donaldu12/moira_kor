...
import org.athomeprojects.moira.MenuFolder;
...
public class ChartData {
    ...
    /// 폰트관련
    private static int swtStyleToAwt(int swtStyle) {
    	int rtn = Font.PLAIN;
    ...
    private void showDesc(DrawAWT draw, String prefix, String year_info,
            ...
            boolean footnote)
    {
    	String fontName = FontMap.getSwtFontName();
    	int fontSize = Resource.getSwtDataFontSize();
    	int swtStyle = MenuFolder.getSwtFontStyle();
    	int awtStyle = swtStyleToAwt(swtStyle);
    	Font awtFont = new Font(fontName,awtStyle,fontSize);
    ...
    private void showData()
    {        
        tab.clear(); // 테이블 데이터를 초기화
        pole_tab.clear(); // 기둥 테이블 데이터를 초기화
        tip.reset(); // 툴팁 데이터를 초기화
        boolean dual_or_single = dual_ring || single_wheel_mode; // 듀얼 링 또는 싱글 휠 모드인지 확인
        day_table = null; // 일별 테이블 데이터를 초기화
        center_tip = birth_sign_tip = now_sign_tip = astro_cusp_data = horiz_sign_data = null; // 중심 툴팁, 출생 별자리 툴팁, 현재 별자리 툴팁, 천궁선 데이터, 수평 별자리 데이터를 초기화
        day_or_night = ""; // 낮/밤 상태를 초기화
        three_danger = null; // 위험 관련 데이터를 초기화
        Arrays.fill(astro_elemental, 0); // 점성학적 요소 배열을 0으로 초기화
        Arrays.fill(astro_elemental_state, 0); // 점성학적 요소 상태 배열을 0으로 초기화
        birth_sign_status_fill = ""; // 출생 별자리 상태를 초기화
        int[] date_buf; // 날짜 버퍼 배열 선언
        int degree_mode = ChartMode.getDegreeMode(quick_azimuth); // 방위각 설정에 따라 각도 모드 가져오기
        boolean return_mode = ChartMode.isReturnMode(); // 차트가 반환 모드인지 확인
        boolean show_house_system = !ChartMode
                .isChartMode(ChartMode.ASTRO_MODE) // 차트가 ASTRO_MODE가 아닌 경우
                && Resource.getPrefInt("show_house_system") != 0 // 사용자 설정에서 하우스 시스템 표시가 활성화된 경우
                && !show_aspects;
        String house_gap = show_house_system ? ("   " + Resource
            .spacePostFilled("", house_name[0].length())) : ""; // 하우스 시스템이 표시되는 경우 하우스 이름의 간격을 설정
        String child_limit = null; // 자녀 제한 관련 변수 초기화
        if (!ChartMode.isChartMode(ChartMode.PICK_MODE) && current_age > 0
            && current_age < 100) { // PICK_MODE가 아니고 나이가 0보다 크고 100보다 작은 경우
            age_label = Integer.toString(current_age)
                + Resource.getString("age"); // 나이를 문자열로 변환하고 "age" 문자열을 추가
        } else {
            age_label = ""; // 나이가 조건에 맞지 않으면 나이 레이블을 빈 문자열로 설정
        }
        day_or_night = Resource.getString(day_birth ? "daytime" : "nighttime"); // 출생 시간이 낮인지 밤인지에 따라 문자열 설정
        String life_helper_key = Resource.getString("life_helper_key"); // 생명 보조 키 문자열 가져오기
        String against_key = Resource.getString("against_key"); // 반대 키 문자열 가져오기
        for (int j = 0; j < (ChartMode.isChartMode(ChartMode.PICK_MODE) ? 1 : 2); j++) { // PICK_MODE인 경우 1회 반복, 그렇지 않으면 2회 반복
            boolean birth_data = j == 0; // 첫 번째 반복에서는 출생 데이터 처리
            double[] sign_pos, sign_alt; // 별자리 위치와 고도 데이터를 저장할 배열 선언
            int[] sign_state; // 별자리 상태 데이터를 저장할 배열 선언
            String[] sign_status; // 별자리 상태 문자열을 저장할 배열 선언
            astro_sign_data[j] = null; // 현재 반복에 해당하는 별자리 데이터를 초기화
            if (birth_data) { // 출생 데이터 처리 중인 경우
                if (Resource.hasCustomData()) { // 사용자 정의 데이터가 있는 경우
                    tab.appendLine(Resource.getString("mod_label") + ": "
                            + Resource.getModName()); // 사용자 정의 데이터의 이름을 테이블에 추가
                }
                if (show_gauquelin // Gauquelin 데이터 표시 옵션이 활성화된 경우
                        || !ChartMode.isAstroMode(ChartMode.NATAL_MODE) // NATAL_MODE가 아닌 경우
                        && !dual_or_single // 듀얼 링 또는 싱글 휠 모드가 아닌 경우
                        || ChartMode.isAstroMode(ChartMode.COMPARISON_MODE)) { // COMPARISON_MODE인 경우
                    tab.appendLine(ChartMode.getModeName(true, true)); // 현재 차트 모드의 이름을 테이블에 추가
                } else if (ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // ASTRO_MODE인 경우
                    tab.appendLine(Resource.getString("birth_chart")); // "birth_chart" 문자열을 테이블에 추가
                }
                DataEntry entry = (return_mode // 반환 모드이거나
                        || ChartMode.isAstroMode(ChartMode.ALT_NATAL_MODE) // ALT_NATAL_MODE인 경우
                        || ChartMode.isAstroMode(ChartMode.RELATIONSHIP_MODE)) ? now // RELATIONSHIP_MODE인 경우 현재 데이터를 사용
                        : birth; // 그렇지 않으면 출생 데이터를 사용
                boolean is_long_lat = updateLocation(entry.getCity(), // 도시와 국가 정보를 사용해 위치를 업데이트
                        entry.getCountry());
                cal.getLocation(birth_loc); // 출생 위치 데이터를 가져옴
                date_buf = entry.getBirthDay(); // 출생 날짜 데이터를 가져옴
                String zone = entry.getZone(); // 시간대 데이터를 가져옴
                BaseCalendar.addZoneOffset(zone, date_buf, 0, false); // 시간대 오프셋을 출생 날짜에 추가
                cal.setJulianDay(date_buf); // 율리우스 날짜를 설정
                if (ChartMode.isAstroMode(ChartMode.COMPARISON_MODE) // COMPARISON_MODE이거나
                        || ChartMode.isAstroMode(ChartMode.COMPOSITE_MODE)) // COMPOSITE_MODE인 경우
                    tab.setName(birth.getName(), birth.getSex(), false); // 이름과 성별 데이터를 테이블에 설정
                else if (ChartMode.isMultipleMode(false)) // 다중 모드인 경우
                    tab.setName(getBirthName(), false); // 출생 이름만 테이블에 설정
                else // 그 외의 경우
                    tab.setName(getBirthName(), getBirthSex(), false); // 출생 이름과 성별을 테이블에 설정
                sign_pos = birth_sign_pos; // 출생 별자리 위치 데이터를 설정
                sign_state = birth_sign_state; // 출생 별자리 상태 데이터를 설정
                sign_status = birth_sign_status; // 출생 별자리 상태 문자열을 설정
                sign_alt = birth_sign_alt; // 출생 별자리 고도 데이터를 설정
                String str; // 임시 문자열 변수 선언
                showLocationInfo(entry, is_long_lat, true, false, 0.0, // 위치 정보를 표시
                        date_buf, birth_base_time); // 날짜 버퍼와 기본 시간을 사용
                String solar_eclipse_info = getEclipseInfo(solar_eclipse, zone, // 태양 일식 정보를 가져옴
                        true, false); // 현재 데이터와 관련된 정보
                String lunar_eclipse_info = getEclipseInfo(lunar_eclipse, zone, // 달 일식 정보를 가져옴
                        false, false); // 현재 데이터와 관련된 정보
                                if (solar_eclipse_info != null || lunar_eclipse_info != null) { // 태양 일식 또는 달 일식 정보가 있는 경우
                    if (solar_eclipse_info != null) // 태양 일식 정보가 있는 경우
                        tab.appendLine(solar_eclipse_info); // 태양 일식 정보를 테이블에 추가
                    if (lunar_eclipse_info != null) // 달 일식 정보가 있는 경우
                        tab.appendLine(lunar_eclipse_info); // 달 일식 정보를 테이블에 추가
                    tab.appendLine(); // 빈 줄을 추가하여 구분
                }
                tab.appendLine(lunar_calendar // 음력 달력 정보를 테이블에 추가
                        + ": "
                        + birth_poles[CH_YEAR] // 출생 연도의 천간과 지지를 추가
                        + year_char // 연도 문자 추가
                        + " "
                        + (lunar_leap_month ? Resource.getString("leap") : "") // 윤달 여부에 따라 "윤" 문자열 추가
                        + BaseCalendar.chineseNumber(lunar_date[1], // 음력 월 숫자를 중국식 숫자로 변환
                                !lunar_leap_month, false)
                        + month_char // 월 문자 추가
                        + " "
                        + BaseCalendar
                                .chineseNumber(lunar_date[2], false, true) // 음력 일 숫자를 중국식 숫자로 변환
                        + day_char + " " // 일 문자 추가
                        + birth_poles[HOUR_POLE].substring(1, 2) + hour_char // 시간의 천간과 지지에서 지지만 추가하고 시간 문자 추가
                        + " (" + day_or_night + "),  " // 낮/밤 정보 추가
                        + Resource.getString("sky_earths") + ": " // "천간지지" 문자열 추가
                        + birth_poles[CH_YEAR] + year_char + " " // 연도의 천간지지 추가
                        + birth_poles[CH_MONTH] + month_char + " " // 월의 천간지지 추가
                        + birth_poles[DAY_POLE] + day_char + " " // 일의 천간지지 추가
                        + birth_poles[HOUR_POLE] + hour_char); // 시간의 천간지지 추가
                tab.append(Resource.getString("eight_characters") + ": " // "팔자" 문자열 추가
                        + birth_poles[YEAR_POLE] + year_char + " " // 연도의 팔자 추가
                        + birth_poles[MONTH_POLE] + month_char + " " // 월의 팔자 추가
                        + birth_poles[DAY_POLE] + day_char + " " // 일의 천간지지와 일 문자를 추가
                        + birth_poles[HOUR_POLE] + hour_char + " [" // 시간의 천간지지와 시간 문자를 추가
                        + getYearSoundName(birth_poles[YEAR_POLE]) + "]"); // 연도의 천간 소리를 가져와 추가
                String birth_season = ""; // 출생 계절 정보를 저장할 변수 초기화
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // ASTRO_MODE가 아닌 경우
                    tab.append(",   " + Resource.getString("weak") // "약한" 별자리 하우스 정보 추가
                            + Resource.getString("zodiac_house") + ": "
                            + weak_house[YEAR_POLE] + weak_house[MONTH_POLE] // 연, 월의 약한 하우스 정보 추가
                            + weak_house[DAY_POLE] + weak_house[HOUR_POLE] // 일, 시간의 약한 하우스 정보 추가
                            + ",   " + Resource.getString("solid") // "강한" 별자리 하우스 정보 추가
                            + Resource.getString("zodiac_house") + ": "
                            + solid_house[YEAR_POLE] + solid_house[MONTH_POLE] // 연, 월의 강한 하우스 정보 추가
                            + solid_house[DAY_POLE] + solid_house[HOUR_POLE]); // 일, 시간의 강한 하우스 정보 추가
                    tab.appendLine(); // 줄바꿈 추가
                    if (ChartMode.isChartMode(ChartMode.PICK_MODE)) { // PICK_MODE인 경우
                        three_danger = computeThreeDanger(); // 삼재(Three Danger) 계산
                        if (three_danger != null) { // 삼재 정보가 있는 경우
                            str = Resource.getString("mountain_name") + ": " // "산 이름" 문자열 추가
                                    + findMountainSign() // 산 별자리 정보를 추가
                                    + Resource.getString("confront"); // "대립" 문자열 추가
                            if (three_danger[YEAR_POLE]) // 연도에 삼재가 있는 경우
                                str += year_char; // 연도 문자 추가
                            if (three_danger[MONTH_POLE]) // 월에 삼재가 있는 경우
                                str += month_char; // 월 문자 추가
                            if (three_danger[DAY_POLE]) // 일에 삼재가 있는 경우
                                str += day_char; // 일 문자 추가
                            if (three_danger[HOUR_POLE]) // 시간에 삼재가 있는 경우
                                str += hour_char; // 시간 문자 추가
                            str += Resource.getString("three_danger"); // "삼재" 문자열 추가
                            tab.appendLine(str); // 삼재 정보를 테이블에 추가
                        }
                    }
                    birth_season = getBirthSeason(birth_adj_date); // 출생 계절 정보를 가져옴
                    if (!ChartMode.isChartMode(ChartMode.PICK_MODE)) // PICK_MODE가 아닌 경우
                        tab.append(Resource.getString("birth_at") + ": "); // "출생 시" 문자열 추가
                    tab.appendLine(birth_season); // 출생 계절 정보를 테이블에 추가
                    if (!ChartMode.isChartMode(ChartMode.PICK_MODE)) { // PICK_MODE가 아닌 경우
                        int day = getChildLimit(false); // 자녀 제한 일수를 계산
                        int[] period = BaseCalendar.timePeriod(date_buf, day); // 날짜 버퍼와 제한 일수를 기반으로 기간 계산
                        str = ""; // 기간 정보를 저장할 문자열 초기화
                        if (period[0] > 0) // 연도 값이 0보다 큰 경우
                            str += Integer.toString(period[0]) + year_char; // 연도 값과 연도 문자를 추가
                        if (period[1] > 0) // 월 값이 0보다 큰 경우
                            str += Integer.toString(period[1]) + month_char; // 월 값과 월 문자를 추가
                        if (period[2] > 0) // 일 값이 0보다 큰 경우
                            str += Integer.toString(period[2]) + day_char; // 일 값과 일 문자를 추가
                        child_limit = Resource.getString("child_limit") + ": " // "자녀 제한" 문자열 추가
                                + str; // 계산된 기간 정보 추가
                        tab.appendLine(child_limit); // 자녀 제한 정보를 테이블에 추가
                    }
                    tab.appendLine(); // 빈 줄 추가
                    printSolarTerms(Resource.isExclusive() // 태양 절기 정보를 출력
                            || Resource.getPrefInt("show_new_moon") == 1); // 특정 조건에 따라 새로운 달 정보를 표시
                }
                tab.appendLine(); // 빈 줄 추가
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // ASTRO_MODE가 아닌 경우
                    for (int i = 0; i < full_stellar_signs.length; i++) { // 모든 별자리 기호를 순회
                        if (Calculate.isValid(life_sign_pos)) { // 별자리 위치가 유효한 경우
                            tab.append(full_stellar_signs[i] // 별자리 이름 추가
                                    + ": "
                                    + cal.formatDegree(stellar_sign_pos[i], // 별자리 위치를 각도로 포맷
                                            true, true));
                            if (Resource.getPrefInt("show_stellar_width") == 1) { // 별자리 폭을 표시하는 설정이 활성화된 경우
                                double width = stellar_sign_pos[(i == full_stellar_signs.length - 1) ? 0 // 별자리 폭 계산
                                        : (i + 1)]
                                        - stellar_sign_pos[i];
                                if (width < 0.0) // 폭이 음수인 경우 360도를 더해 양수로 변환
                                    width += 360.0;
                                tab.append(" [" // 별자리 폭 정보를 추가
                                        + FileIO.formatDouble(width, 2, 2,
                                                true, false)
                                        + "]");
                            }
                        }
                        if ((i % 4) == 3) // 4개의 별자리마다 줄바꿈 추가
                            tab.appendLine(); // 줄바꿈 추가
                        else
                            tab.append("  "); // 별자리 간 간격 추가
                    }
                    tab.appendLine(); // 줄바꿈 추가
                    if (Resource.getPrefInt("show_stellar_width") == 1) { // 별자리 폭 표시 설정이 활성화된 경우
                        for (int i = 0; i < full_zodiac.length; i++) { // 모든 황도대 기호를 순회
                            tab.append(full_zodiac[i] // 황도대 이름 추가
                                    + ": "
                                    + cal.formatDegree(i * 30.0, // 각도를 30도 단위로 포맷
                                            stellar_sign_pos,
                                            full_stellar_signs, true));
                            if ((i % 4) == 3) // 4개의 황도대마다 줄바꿈 추가
                                tab.appendLine(); // 줄바꿈 추가
                            else
                                tab.append("  "); // 황도대 간 간격 추가
                        }
                        tab.appendLine(); // 줄바꿈 추가
                        if (show_compass && Calculate.isValid(life_sign_pos)) { // 나침반 표시 설정이 활성화되고 생명 별자리 위치가 유효한 경우
                            for (int i = 0; i < full_stellar_signs.length; i++) { // 모든 별자리 기호를 순회
                                tab.append(full_stellar_signs[i] // 별자리 이름 추가
                                        + ": "
                                        + cal.formatDegree(compass_sign_pos[i], // 나침반 별자리 위치를 각도로 포맷
                                                true, true));
                                double width = compass_sign_pos[(i == full_stellar_signs.length - 1) ? 0 // 나침반 별자리 폭 계산
                                        : (i + 1)]
                                        - compass_sign_pos[i];
                                if (width < 0.0) // 폭이 음수인 경우 360도를 더해 양수로 변환
                                    width += 360.0;
                                tab.append(" [" // 나침반 별자리 폭 정보를 추가
                                        + FileIO.formatDouble(width, 2, 2,
                                                true, false)
                                        + "]");
                                if ((i % 4) == 3) // 4개의 별자리마다 줄바꿈 추가
                    ...
                    str = ChartMode.getModeName(true, true); // 현재 차트 모드의 이름을 가져옴
                    if (str != null) // 차트 모드 이름이 null이 아닌 경우
                        tab.appendLine(str); // 차트 모드 이름을 테이블에 추가
                    tab.appendLine(ChartMode.getComputationMethod()); // 계산 방법 정보를 테이블에 추가
                    if (Resource.getPrefInt("use_equator") != 0) // 적도 사용 설정이 활성화된 경우
                        tab.appendLine(Resource.getString("equator_sunrise")); // "적도 일출" 문자열을 테이블에 추가
                    tab.appendLine(); // 빈 줄 추가
                    if (show_house_system) { // 하우스 시스템 표시 설정이 활성화된 경우
                        tab.appendLine(ChartMode.getSystemName(ChartMode // 현재 하우스 시스템 이름을 가져와 추가
                                .isChartMode(ChartMode.PICK_MODE) ? "pick_" // PICK_MODE인 경우 "pick_" 접두사 추가
                                        : ""));
                        showCuspData(birth_cusp, house_name); // 출생 하우스와 하우스 이름 정보를 표시
                    }
                } else { // 위 조건이 만족되지 않는 경우
                    tab.appendLine(); // 빈 줄 추가
                    if (ChartMode.isAstroMode(ChartMode.COMPOSITE_MODE)) { // 현재 차트 모드가 COMPOSITE_MODE인 경우
                        is_long_lat = updateLocation(now.getCity(), // 현재 위치 정보를 업데이트
                                now.getCountry());
                        cal.getLocation(now_loc); // 현재 위치 좌표를 가져옴
                        date_buf = now.getBirthDay(); // 현재 생일 정보를 가져옴
                        zone = now.getZone(); // 현재 시간대를 가져옴
                        BaseCalendar.addZoneOffset(zone, date_buf, 0, false); // 시간대 오프셋을 추가
                        cal.setJulianDay(date_buf); // 율리우스 날짜를 설정
                        tab.setName(now.getName(), now.getSex(), false); // 이름과 성별 정보를 테이블에 추가
                        now_base_time = getAdjustDate(zone, now_loc[0], // 조정된 현재 날짜와 시간 계산
                                date_buf, now_adj_date, now_dst_date);
                        showLocationInfo(now, is_long_lat, false, false, 0.0, // 위치 정보를 표시
                                date_buf, now_base_time);
                    }
                    String s_str = ChartMode.getSidrealSystem(); // 항성 시스템 정보를 가져옴
                    if (s_str != null) // 항성 시스템 정보가 null이 아닌 경우
                        tab.appendLine(s_str); // 항성 시스템 정보를 테이블에 추가
                    tab.appendLine(ChartMode.getComputationMethod()); // 계산 방법 정보를 테이블에 추가
                    tab.appendLine(); // 빈 줄 추가
                    tab.appendLine(ChartMode.getSystemName("")); // 하우스 시스템 이름을 테이블에 추가
                    showCuspData(birth_cusp, cusps_name); // 출생 하우스와 하우스 이름 정보를 표시
                }
                center_tip = ""; // 중심 팁 정보를 초기화
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // 현재 차트 모드가 ASTRO_MODE가 아닌 경우
                    if (showStyles(good_styles, "good_styles") // 좋은 스타일을 표시
                            && showStyles(bad_styles, "bad_styles")) // 나쁜 스타일을 표시
                        tab.appendLine(); // 빈 줄 추가
                    str = Resource.getString("life_master") // "생명 주인" 문자열 가져오기
                            + ": "
                            + cal.formatDegree(life_sign_pos, stellar_sign_pos, // 생명 별자리 위치를 각도로 포맷
                                    full_stellar_signs, "  ", false, true);
                    tab.append(str + "  "); // 생명 주인 정보를 테이블에 추가
                    center_tip += str; // 중심 팁에 생명 주인 정보 추가
                    str = Resource.getString("self_master") // "자신 주인" 문자열 가져오기
                            + ": "
                            + cal.formatDegree(self_sign_pos, stellar_sign_pos, // 자신 별자리 위치를 각도로 포맷
                                    full_stellar_signs, "  ", false, true);
                    tab.appendLine(str); // 자신 주인 정보를 테이블에 추가
                    center_tip += "\n" + str; // 중심 팁에 자신 주인 정보 추가
                    if (ChartMode.isChartMode(ChartMode.PICK_MODE)) { // 현재 차트 모드가 PICK_MODE인 경우
                        center_tip += "\n"
                                + Resource.getString("mountain_name") // "산 이름" 문자열 가져오기
                                + ": "
                                + cal.formatDegree(mountain_pos // 산 위치를 각도로 포맷
                                        - true_north_magnetic_shift, // 진북 자기 이동값을 보정
                                        stellar_sign_pos, full_stellar_signs,
                                        "  ", true, false);
                        if (three_danger != null) { // 삼재 정보가 존재하는 경우
                            center_tip += ",  "
                                    + Resource.getString("confront"); // "대립" 문자열 추가
                            if (three_danger[YEAR_POLE]) // 연간 삼재가 있는 경우
                                center_tip += year_char; // 연도 문자 추가
                            if (three_danger[MONTH_POLE]) // 월간 삼재가 있는 경우
                                center_tip += month_char; // 월 문자 추가
                            if (three_danger[DAY_POLE]) // 일간 삼재가 있는 경우
                                center_tip += day_char; // 일 문자 추가
                            if (three_danger[HOUR_POLE]) // 시간 삼재가 있는 경우
                                center_tip += hour_char; // 시간 문자 추가
                            center_tip += Resource.getString("three_danger"); // "삼재" 문자열 추가
                        }
                    }
                    String alt_key = Resource.getString("alt_life_helper_key"); // 대체 생명 도움 키 문자열 가져오기
                    for (int iter = 0; iter < (ChartMode // 반복 횟수 결정 (PICK_MODE인 경우 3, 그렇지 않으면 2)
                            .isChartMode(ChartMode.PICK_MODE) ? 3 : 2); iter++) {
                        String sign, name; // 별자리와 이름 변수 선언
                        switch (iter) { // 반복 인덱스에 따라 처리
                            case 0: // 첫 번째 반복
                                sign = cal.getStarSign(life_sign_pos, // 생명 별자리 위치를 기반으로 별자리 가져오기
                                        stellar_sign_pos, full_stellar_signs);
                                name = Resource.getString("life_master_name"); // "생명 주인 이름" 문자열 가져오기
                                break;
                            case 1: // 두 번째 반복
                                sign = cal.getZodiac(life_sign_pos, true); // 생명 별자리 위치를 기반으로 황도대 기호 가져오기
                                name = Resource.getString("life_sign_name"); // "생명 별자리 이름" 문자열 가져오기
                                break;
                            default: // PICK_MODE인 경우 세 번째 반복
                                sign = findMountainSign(); // 산 별자리 기호 가져오기
                                name = Resource.getString("mountain_name"); // "산 이름" 문자열 가져오기
                                break;
                        }
                        String[] helper = Resource // 생명 도움 배열 가져오기
                                .getStringArray(life_helper_key
                                        + sign.substring(1)); // 별자리 기호의 두 번째 문자부터 키 생성
                        str = ""; // 도움 문자열 초기화
                        for (int i = 0; i < helper.length; i++) { // 도움 배열 순회
                            if (i > 0) // 첫 번째 요소가 아닌 경우
                                str += ", "; // 구분자 추가
                            if (ChartMode.isChartMode(ChartMode.PICK_MODE)) // PICK_MODE인 경우
                                str += alt_key.substring(i, i + 1); // 대체 키에서 문자 추가
                            else // 그렇지 않은 경우
                                str += life_helper_key.substring(i, i + 1); // 기본 키에서 문자 추가
                            str += ":" + helper[i]; // 도움 정보 추가
                        }
                        tab.appendLine(name + ":" + sign + "  " + str); // 이름, 별자리, 도움 정보를 테이블에 추가
                        center_tip += "\n" + name.substring(1) + ": " + str; // 중심 팁에 도움 정보 추가
                    }
                    if (ChartMode.isChartMode(ChartMode.PICK_MODE)) { // PICK_MODE인 경우
                        String[] helper_season = Resource // 계절 도움 배열 가져오기
                                .getStringArray("season_helper_season");
                        int helper_index = getSeasonIndex(birth_adj_date); // 조정된 생일을 기반으로 계절 인덱스 가져오기
                        str = helper_season[helper_index].replaceFirst("-", // 계절 도움 문자열에서 첫 번째 "-"를 ": "로 대체
                                ": ");
                        tab.appendLine(str); // 계절 도움 문자열을 테이블에 추가
                        center_tip += "\n" + str; // 중심 팁에 계절 도움 문자열 추가
                    }
                    center_tip += "\n" + Resource.getString("weak") + ": " // "약한" 하우스 정보 추가
                            + weak_house[YEAR_POLE] + weak_house[MONTH_POLE] // 연, 월, 일, 시 약한 하우스 정보 추가
                            + weak_house[DAY_POLE] + weak_house[HOUR_POLE]
                            + ", " + Resource.getString("solid") + ": " // "단단한" 하우스 정보 추가
                            + solid_house[YEAR_POLE] + solid_house[MONTH_POLE] // 연, 월, 일, 시 단단한 하우스 정보 추가
                            + solid_house[DAY_POLE] + solid_house[HOUR_POLE];
                    center_tip += "\n"; // 줄바꿈 추가
                }
                if (!dual_or_single) { // 단일 모드가 아닌 경우
                    center_tip += lunar_calendar // 음력 달력 정보 추가
                            + ": "
                            + birth_poles[CH_YEAR] // 출생 연도 정보 추가
                            + year_char
                            + (lunar_leap_month ? Resource.getString("leap") // 윤달 여부 확인 후 "윤달" 문자열 추가
                                    : "")
                            + BaseCalendar.chineseNumber(lunar_date[1], // 음력 월 정보 추가
                                    !lunar_leap_month, false)
                            + month_char
                            + BaseCalendar.chineseNumber(lunar_date[2], false, // 음력 일 정보 추가
                                    true)
                            + day_char + " (" + day_or_night // 낮/밤 정보 추가
                            + ")\n";
                    if (solar_eclipse_info != null // 일식 정보가 있는 경우
                            || lunar_eclipse_info != null) { // 월식 정보가 있는 경우
                        if (solar_eclipse_info != null) // 일식 정보가 null이 아닌 경우
                            center_tip += solar_eclipse_info + "\n"; // 일식 정보를 중심 팁에 추가
                        if (lunar_eclipse_info != null) // 월식 정보가 null이 아닌 경우
                            center_tip += lunar_eclipse_info + "\n"; // 월식 정보를 중심 팁에 추가
                    }
                }
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // 현재 차트 모드가 ASTRO_MODE가 아닌 경우
                    if (!ChartMode.isChartMode(ChartMode.PICK_MODE)) // PICK_MODE가 아닌 경우
                        center_tip += Resource.getString("birth_at") + ": "; // "출생 위치" 문자열 추가
                    center_tip += birth_season; // 출생 계절 정보 추가
                    if (!ChartMode.isChartMode(ChartMode.PICK_MODE)) { // PICK_MODE가 아닌 경우
                        if (!age_label.equals("")) // 나이 레이블이 비어있지 않은 경우
                            center_tip += "  " + age_label; // 나이 레이블 추가
                        if (child_limit != null) // 자녀 제한 정보가 존재하는 경우
                            center_tip += "  " + child_limit; // 자녀 제한 정보 추가
                    }
                }
                } else { // ASTRO_MODE인 경우
                    boolean is_long_lat = updateLocation(now.getCity(), // 현재 도시와 국가 정보를 기반으로 위치 업데이트
                            now.getCountry());
                    cal.getLocation(now_loc); // 현재 위치 정보를 가져옴
                    date_buf = (dual_or_single || ChartMode // 날짜 버퍼 설정
                            .isAstroMode(ChartMode.COMPOSITE_MODE)) ? now.getBirthDay() // 복합 모드인 경우 출생일 사용
                            : now.getNowDay(); // 그렇지 않으면 현재 날짜 사용
                    String zone = now.getZone(); // 현재 시간대 가져오기
                    BaseCalendar.addZoneOffset(zone, date_buf, 0, false); // 시간대 오프셋 추가
                    cal.setJulianDay(date_buf); // 율리우스 날짜 설정
                    sign_pos = now_sign_pos; // 현재 별자리 위치 설정
                    sign_state = now_sign_state; // 현재 별자리 상태 설정
                    sign_status = now_sign_status; // 현재 별자리 상태 정보 설정
                    sign_alt = now_sign_alt; // 현재 별자리 고도 설정
                    if (dual_or_single) { // 단일 모드 또는 이중 모드인 경우
                        tab.appendLine(); // 빈 줄 추가
                        tab.appendLine(); // 빈 줄 추가
                        if (ChartMode.isAstroMode(ChartMode.COMPARISON_MODE)) // 비교 모드인 경우
                            tab.setName(now.getName(), now.getSex(), false); // 이름과 성별 설정
                        else
                            tab.appendLine(ChartMode.getModeName(true, true)); // 차트 모드 이름 추가
                        now_base_time = getAdjustDate(zone, now_loc[0], date_buf, // 조정된 현재 시간 가져오기
                                now_adj_date, now_dst_date);
                        boolean show_time = ChartMode // 시간을 표시해야 하는지 확인
                                .isAstroMode(ChartMode.SECONDARY_PROGRESSION_MODE)
                                || ChartMode.isAstroMode(ChartMode.SOLAR_ARC_MODE);
                                showLocationInfo(now, is_long_lat, false, show_time, // 현재 위치 정보 표시 (추가 매개변수 포함)
                                        now_ref_ut, date_buf, now_base_time);
                                }
                                if (show_now) { // 현재 정보를 표시해야 하는 경우
                                    tab.appendLine(); // 빈 줄 추가
                                    tab.appendLine(now_year + ":"); // 현재 연도 추가
                                    String str = now_poles[YEAR_POLE]; // 현재 연도의 천간과 지지 가져오기
                                    if (!age_label.equals("")) // 나이 레이블이 비어있지 않은 경우
                                        str += "  " + age_label; // 나이 레이블 추가
                                    str += "  "
                                            + lunar_calendar // 음력 달력 정보 추가
                                            + ": "
                                            + (now_lunar_leap_month ? Resource // 현재 음력 윤달 여부 확인
                                                    .getString("leap") : "")
                                            + BaseCalendar.chineseNumber(now_lunar_date[1], // 현재 음력 월 정보 추가
                                                    !now_lunar_leap_month, false)
                                            + month_char
                                            + BaseCalendar.chineseNumber(now_lunar_date[2], // 현재 음력 일 정보 추가
                                                    false, true) + day_char + "  ";
                                    str += "limit"; // 제한 정보 (나중에 대체 예정)
                                    tab.appendLine(str); // 현재 연도 정보 추가
                                    center_tip += "\n" // 중심 팁에 현재 연도 정보 추가
                                            + now_year
                                            + ": "
                                            + now_poles[YEAR_POLE]
                                            ...
                                            + BaseCalendar.chineseNumber(now_lunar_date[2], // 현재 음력 일 정보 추가
                                                    false, true) + day_char + "\n";
                                    int child_age_limit = getChildLimit(true); // 자녀 나이 제한 가져오기
                                    if (current_age > 1 && current_age - 1 < child_age_limit) // 현재 나이가 1보다 크고 자녀 나이 제한보다 작은 경우
                                        center_tip += getChildLimit(current_age - 1, ": ")
                                                + ", ";
                                         center_tip += getSmallLimit(current_age, ": ") + ", "
                                            + getMonthLimit(current_age, ": ");
                            ...
                            String[] status_seq = Resource.getStringArray("sign_status_seq"); // 별자리 상태 시퀀스 문자열 배열 가져오기
                            int[] seq_freq = new int[status_seq.length]; // 상태 시퀀스 빈도 배열 초기화
                            Arrays.fill(seq_freq, 0); // 빈도 배열을 0으로 초기화
                            for (int i = 0; i < signs.length; i++) { // 모든 별자리에 대해 반복
                                if ((birth_data || dual_or_single || show_now) // 출생 데이터, 단일/이중 모드, 현재 표시 여부 확인
                                        && Calculate.isValid(sign_pos[i])) { // 별자리 위치가 유효한 경우
                                    sign_status[i] = getSignStatus(signs[i], sign_pos[i], // 별자리 상태 계산
                                            status_seq, seq_freq);
                                }
                            }
                            String sign_tip = ""; // 별자리 팁 문자열 초기화
                            if (!show_horiz) { // 수평선 표시가 비활성화된 경우
                                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // 현재 차트 모드가 ASTRO_MODE가 아닌 경우
                                    sign_tip += Resource.getString("life_master") + "     : "; // "생명 주인" 문자열 추가
                                    if (show_house_system) // 하우스 시스템 표시 여부 확인
                                        sign_tip += house_gap; // 하우스 간격 추가
                                    sign_tip += cal.formatDegree(life_sign_pos, // 생명 별자리 위치 포맷팅
                                            stellar_sign_pos, full_stellar_signs, "  ", false, true);
                                    if (Calculate.isValid(self_sign_pos)) { // 자기 별자리 위치가 유효한 경우
                                        sign_tip += "\n" + Resource.getString("self_master") // "자기 주인" 문자열 추가
                                                + "     : ";
                                        if (show_house_system) // 하우스 시스템 표시 여부 확인
                                            sign_tip += house_gap; // 하우스 간격 추가
                                        sign_tip += cal.formatDegree(self_sign_pos, // 자기 별자리 위치 포맷팅
                                                stellar_sign_pos, full_stellar_signs, "  ", false, true);
                                    }
                                }
                                if (ChartMode.isChartMode(ChartMode.PICK_MODE)) { // 현재 차트 모드가 PICK_MODE인 경우
                                    now_sign_tip = Resource // 현재 별자리 팁 문자열 설정
                                            .getString(quick_azimuth ? "quick_azimuth_method" // 빠른 방위각 방법 여부 확인
                                                    : "local_horizon_method")
                                            + "    " + azimuth_speed; // 방위각 속도 추가
                                    sign_tip = Resource.getString("tropical_method") + "\n" // "열대 방법" 문자열 추가
                                            + sign_tip;
                                    if (quick_azimuth) { // 빠른 방위각 방법이 활성화된 경우
                        tab.appendLine();
                        tab.appendLine(Resource // "quick_azimuth_method" 리소스 문자열과 "pick_" 시스템 이름을 추가
                                .getString("quick_azimuth_method")
                                + ChartMode.getSystemName("pick_"));
                        for (int i = 0; i < cusps_name.length; i++) { // 커스프 이름 배열 순회
                            String str = cusps_name[i] // 커스프 이름과 해당 각도를 포맷팅하여 문자열 생성
                                    + ": "
                                    + cal.formatDegree(birth_cusp[i + 1], true, true);
                            tab.append(str); // 문자열을 탭에 추가
                            if ((i % 2) == 1) { // 인덱스가 홀수인 경우 줄바꿈 추가
                                tab.appendLine();
                            } else {
                                tab.append("  "); // 그렇지 않으면 공백 추가
                            }
                        }
                        tab.appendLine(); // 마지막 줄바꿈 추가
                    }
                    tab.appendLine(Resource.getString("tropical_method") // "tropical_method" 리소스 문자열 추가
                            + "                              "
                            + formatStatusString("", status_seq, seq_freq) // 상태 문자열 포맷팅
                                    .replaceAll("@", "  ") // "@"를 공백으로 대체
                            + house_gap // 하우스 간격 추가
                            + Resource // "quick_azimuth_method" 또는 "local_horizon_method" 문자열 추가
                                    .getString(quick_azimuth ? "quick_azimuth_method"
                                            : "local_horizon_method")
                            + "    "
                            + azimuth_speed); // 방위각 속도 추가
            ...
            Hashtable table = birth_data ? birth_table : now_table; // 출생 데이터 여부에 따라 테이블 선택
            for (int i = 0; i < signs.length; i++) { // 별자리 배열 순회
                int order = birth_data ? birth_sign_display_sort_orders[i] // 정렬 순서 결정
                        : now_sign_display_sort_orders[i];
                if (!(birth_data || dual_or_single || show_now) // 특정 조건을 만족하지 않거나
                        || !Calculate.isValid(sign_pos[order])) // 별자리 위치가 유효하지 않은 경우
                    continue; // 다음 반복으로 건너뜀
                String prefix = signs[order]; // 현재 별자리 이름을 접두사로 설정
                if (table != null) { // 테이블이 존재하는 경우
                    String key = (table != null) ? ((String) table // 테이블이 null이 아닌 경우, 현재 별자리에 해당하는 키를 가져옴
                            .get(signs[order])) : null;
                    if (key != null) // 키가 존재하면
                        prefix += "(" + key + "): "; // 접두사에 키를 추가
                    else
                        prefix += "       : "; // 키가 없으면 공백 추가
                } else {
                    prefix += ": "; // 테이블이 null인 경우 기본 접두사 추가
                }
                String str, m_str, gauquelin; // 문자열 변수 초기화
                double curDegree = -1; // 현재 별의 락도
                if (show_horiz) { // 수평 좌표계를 표시해야 하는 경우
                    str = prefix
                            + cal.formatDegree(birth_sign_azimuth[order], // 별자리 방위각을 각도로 포맷팅
                                    birth_sign_alt[order], cal
                                            .getSpeedStateName(
                                                    sign_state[order], "  ")) // 별자리 상태 이름 추가
                            + " ";
                    m_str = gauquelin = ""; // 추가 문자열 초기화
                } else { // 수평 좌표계를 표시하지 않는 경우
                	
                    str = prefix; // 접두사만 추가
                    if (show_house_system) { // 하우스 시스템을 표시해야 하는 경우
                        int val = cal.getSignIndex(sign_pos[order], birth_cusp, // 별자리 위치와 커스프를 기반으로 인덱스 계산
                                1);
                        if (val >= 1) { // 유효한 인덱스인 경우
                            str += "[" + house_name[val - 1] + "] "; // 하우스 이름 추가
                        } else { // 유효하지 않은 경우
                            str += "   "
                                    + Resource.spacePostFilled("", // 공백으로 채움
                                            house_name[0].length());
                        }
                    }
                    str += cal.formatDegree(sign_pos[order], stellar_sign_pos, // 별자리 위치를 각도로 포맷팅하여 문자열에 추가
                            full_stellar_signs, // 전체 별자리 기호 사용
                            cal.getSpeedStateName(sign_state[order], "  "), // 별자리 상태 이름 가져오기
                            false, true)
                            + " ";
                    curDegree = cal.formatDegreeOh(sign_pos[order], stellar_sign_pos, // 별자리 위치를 각도로 포맷팅하여 문자열에 추가
                            full_stellar_signs, // 전체 별자리 기호 사용
                            cal.getSpeedStateName(sign_state[order], "  "), // 별자리 상태 이름 가져오기
                            false, true);
                    
                    if (ChartMode.isChartMode(ChartMode.TRADITIONAL_MODE) // 전통 모드 또는 항성 모드인 경우
                            || ChartMode.isChartMode(ChartMode.SIDEREAL_MODE)) {
                        if (Calculate.isValid(sign_alt[order])) { // 별자리 고도가 유효한 경우
                            str += FileIO.formatDouble(sign_alt[order], 2, 1, // 고도를 소수점 2자리로 포맷팅
                                    true, true) + " ";
                        } else {
                            str += "      "; // 고도가 유효하지 않으면 공백 추가
                        }
                    }
                    m_str = show_aspects ? getAspects( // 측면 표시가 활성화된 경우 측면 정보 가져오기
                            ChartMode.isChartMode(ChartMode.ASTRO_MODE) ? "\n    " // 천문 모드일 경우 줄바꿈 추가
                                    : "\n        ",
                            order, birth_data)
                            : null;
                    if (m_str == null) { // 측면 정보가 없는 경우
                        m_str = "";
                        if (sign_state[order] == Calculate.SPEED_REVERSE // 별자리 상태가 역행 중이고
                                && !has_pos_adj) { // 위치 조정이 없는 경우
                            double ut_sav = cal.getJulianDayUT(); // 현재 율리우스 날짜 저장
                            double ut = cal.computeSpeedTransit(planets[order], // 역행 종료 시점 계산
                                    ut_sav, 0.0, false);
                            if (Calculate.isValid(ut)) { // 계산된 값이 유효한 경우
                                cal.setJulianDay(ut); // 역행 종료 시점으로 날짜 설정
                                double degree = cal.compute(planets[order]); // 해당 시점의 별자리 위치 계산
                                cal.setJulianDay(ut_sav); // 원래 날짜로 복원
                                m_str = "\n  "
                                        + Resource.getString("retreat_to") // "역행 종료" 문자열 추가
                                        + ": "
                                        + cal.formatDegree(degree, // 종료 시점의 별자리 위치를 각도로 포맷팅
                                                stellar_sign_pos,
                                                full_stellar_signs, "  ",
                                                false, true);
                            }
                    ...
                    if (table != null) { // 별자리 테이블이 존재하는 경우
                        String star_sign_key = Resource // 별자리 키 문자열 가져오기
                                .getString("star_sign_key");
                        LinkedList star_list = (LinkedList) table // 테이블에서 현재 별자리에 해당하는 리스트 가져오기
                                .get(signs[order] + star_sign_key);
                        if (star_list != null) { // 별자리 리스트가 존재하는 경우
                            int count = 0; // 별자리 카운터 초기화
                            for (ListIterator iter = star_list.listIterator(); iter // 리스트 순회
                                    .hasNext();) {
                                String val = (String) iter.next(); // 리스트의 다음 값을 가져오기
                                if (((count++) % 7) == 0) // 7번째 항목마다 줄바꿈 추가
                                    m_str += "\n  ";
                                else
                                    m_str += ", "; // 그렇지 않으면 쉼표 추가
                                m_str += val; // 별자리 값을 문자열에 추가
                            }
                        }
                    }
                    gauquelin = getGauquelin(order); // Gauquelin 데이터 가져오기
                }
                tip.addTip(order, birth_data, str + sign_status[order] // 팁에 별자리 상태와 Gauquelin 데이터 추가
                        + gauquelin + m_str);
                ...
                if(curDegree >= 0) {                
                	for (int dcnt = 0; dcnt < signs.length; dcnt++) {
                	
	                	int n_order = birth_data ? birth_sign_display_sort_orders[dcnt]// 정렬 순서 결정
	                            : now_sign_display_sort_orders[dcnt];
	                    if (!(birth_data || dual_or_single || show_now) // 특정 조건을 만족하지 않거나
	                            || !Calculate.isValid(sign_pos[n_order])) // 별자리 위치가 유효하지 않은 경우
	                        continue; // 다음 반복으로 건너뜀
	                    
	                    if(order == n_order) //같은 별에 대해서는 계산하지 않음
	                		continue;
	                	
	                    double tmp = cal.formatDegreeOh(sign_pos[n_order], stellar_sign_pos, // 별자리 위치를 각도로 포맷팅하여 문자열에 추가
	                            full_stellar_signs, // 전체 별자리 기호 사용
	                            cal.getSpeedStateName(sign_state[n_order], "  "), // 별자리 상태 이름 가져오기
	                            false, true);
	                            
	                    if(curDegree - tmp < 2 && curDegree - tmp > -2){
	                    	dstr += "\n       "+signs[n_order];
	                    	dstr += "   " + cal.formatDegree(sign_pos[n_order], stellar_sign_pos, // 별자리 위치를 각도로 포맷팅하여 문자열에 추가
	                                full_stellar_signs, // 전체 별자리 기호 사용
	                                cal.getSpeedStateName(sign_state[n_order], "  "), // 별자리 상태 이름 가져오기
	                                false, true)
	                                + " ";
                	...
                	if(dstr != "[同絡]"){
                		tip.addTipOh(order, birth_data, dstr);
                	}
                }
                if (birth_data && !ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // 출생 데이터가 있고 ASTRO_MODE가 아닌 경우
                    int index = cal.getSignIndex(sign_pos[order], // 별자리 인덱스 계산
                            stellar_sign_pos, 0);
                    if (index >= 0) { // 유효한 인덱스인 경우
                        String stellar = full_stellar_signs[index]; // 별자리 문자열 가져오기
                        String stellar_sign = stellar.substring(1); // 별자리 이름 추출
                        for (int k = 0; k < full_stellar_signs.length; k++) { // 전체 별자리 배열 순회
                            int offset = (k == index) ? CONSTELLATION_MAIN_OFFSET // 메인 별자리와 서브 별자리 오프셋 설정
                                    : CONSTELLATION_SUB_OFFSET;
                            if (stellar_sign.equals(full_stellar_signs[k] // 현재 별자리가 대응 별자리인지 확인
                                    .substring(1))) {
                                 String t_str = tip.getTip(offset + k, true); // 팁에서 현재 오프셋에 해당하는 문자열 가져오기
                                if (t_str != null) { // 기존 팁 문자열이 있는 경우
                                    t_str += ", " + signs[order]; // 기존 팁 문자열에 현재 별자리 추가
                                } else {
                                    t_str = signs[order]; // 팁 문자열이 없으면 현재 별자리로 초기화
                                }
                                if (Resource.hasKey(against_key // 대응 키가 존재하는 경우
                                        + stellar.substring(1))) {
                                    String[] against = Resource // 대응 별자리 배열 가져오기
                                            .getStringArray(against_key + stellar.substring(1));
                                    for (int l = 0; l < against.length; l++) { // 대응 별자리 배열 순회
                                        String key = against[l];
                                        if (key.equals(signs[order])) { // 현재 별자리가 대응 별자리에 포함되는 경우
                                            t_str += "[" + Resource // 팁 문자열에 "대응 별자리" 표시 추가
                                                    .getString("against_constellation") + "]";
                                            break;
                                        }
                                    }
                                }
                                tip.addTip(offset + k, true, t_str); // 팁에 업데이트된 문자열 추가
                            }
                        }
                    }
                }
                sign_status[order] = formatStatusString(sign_status[order], // 별자리 상태 문자열 포맷팅
                        status_seq, seq_freq);
                String s_str = str; // 현재 문자열을 임시 변수에 저장
                str += sign_status[order].replaceAll("@", "  "); // 상태 문자열에서 "@"를 공백으로 대체
                if (!sign_tip.equals("")) // 기존 별자리 팁이 비어있지 않은 경우
                    sign_tip += "\n"; // 줄바꿈 추가
                sign_tip += str + gauquelin; // 별자리 팁에 현재 문자열과 Gauquelin 데이터 추가
                tab.append(str + gauquelin); // 탭에 현재 문자열과 Gauquelin 데이터 추가
                if (!show_horiz && ChartMode.isChartMode(ChartMode.PICK_MODE)) { // 수평선 표시가 비활성화되고 PICK_MODE인 경우
                    boolean astrolog_coord = Resource // 천문 좌표 설정 여부 확인
                            .getPrefInt("astrolog_coord") != 0;
                    str = prefix // 접두사 문자열에 추가
                            + cal.formatDegree(birth_sign_azimuth[order], "  ", // 출생 별자리 방위각을 포맷팅
                                    show_compass ? compass_sign_pos : null, // 나침반 표시 여부에 따라 나침반 위치 사용
                                    full_stellar_signs, // 전체 별자리 기호 사용
                                    ChartMode.mountainBased(degree_mode), // 산 기반 차트 모드 확인
                                    astrolog_coord); // 천문 좌표 설정 여부
                    if (Calculate.isValid(birth_sign_alt[order])) { // 출생 별자리 고도가 유효한 경우
                        str += " "
                                + FileIO.formatDouble(birth_sign_alt[order], 2, // 고도를 소수점 2자리로 포맷팅
                                        1, true, true);
                    }
                    tab.append("  " + str); // 문자열을 탭에 추가
                    tip.addTip(order, false, str); // 팁에 현재 별자리 정보 추가
                    now_sign_tip += "\n" + str; // 현재 별자리 팁에 문자열 추가
                }
                if (show_horiz) { // 수평선 표시가 활성화된 경우
                    if (horiz_sign_data == null) { // 수평 별자리 데이터가 초기화되지 않은 경우
                        horiz_sign_data = s_str + "|"; // 데이터 초기화
                    } else {
                        horiz_sign_data += s_str + "|"; // 기존 데이터에 추가
                    }
                } else if (ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // ASTRO_MODE인 경우
                    if (astro_sign_data[j] == null) { // 천문 별자리 데이터가 초기화되지 않은 경우
                        astro_sign_data[j] = str + "|"; // 데이터 초기화
                    } else {
                        astro_sign_data[j] += str + "|"; // 기존 데이터에 추가
                    }
                    double pos = single_wheel_mode ? now_sign_pos[order] // 현재 별자리 위치 또는 출생 별자리 위치 사용
                            : birth_sign_pos[order];
                    if (single_wheel_mode && !birth_data || !single_wheel_mode) { // 특정 조건에 따라 원소 및 상태 값 증가
                        astro_elemental[cal.getElementalIndex(pos)]++; // 원소 값 증가
                        astro_elemental_state[cal.getElementalStateIndex(pos)]++; // 상태 값 증가
                    }
                }
                tab.appendLine(); // 줄바꿈 추가
            }
            String sign_status_fill = formatStatusString("", status_seq, // 상태 문자열 포맷팅
                    seq_freq);
            if (birth_data) { // 출생 데이터인 경우
                birth_sign_status_fill = sign_status_fill; // 출생 별자리 상태 채우기 업데이트
                birth_sign_tip = sign_tip; // 출생 별자리 팁 업데이트
            } else {
                if (!dual_ring && ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // 이중 링이 아니고 ASTRO_MODE인 경우
                    tab.appendLine(); // 빈 줄 추가
                    tab.appendLine(); // 빈 줄 추가
                    String[] elementals = Resource // 네 가지 원소 이름 배열 가져오기
                            .getStringArray("elemental_names");
                    for (int i = 0; i < 4; i++) { // 네 가지 원소에 대해 반복
                        tab.append(elementals[i] + ": " // 원소 이름과 해당 값 추가
                                + FileIO.formatInt(astro_elemental[i], 2) + " ");
                    }
                    tab.appendLine(); // 줄바꿈 추가
                    String[] states = Resource // 원소 상태 이름 배열 가져오기
                            .getStringArray("elemental_states");
                    for (int i = 0; i < 3; i++) { // 세 가지 원소 상태에 대해 반복
                        tab.append(states[i] + ": " // 원소 상태 이름과 해당 값 추가
                                + FileIO.formatInt(astro_elemental_state[i], 2) + " ");
                    }
                }
                if (show_aspects) { // 천문학적 관계(aspects)를 표시해야 하는 경우
                    Object[] aspects = getAspectsTable( // 천문학적 관계 테이블 가져오기
                            single_wheel_mode ? now_sign_pos : birth_sign_pos, // 현재 별자리 위치 또는 출생 별자리 위치 사용
                            0, "$", true);
                    if (aspects != null) { // 천문학적 관계 데이터가 있는 경우
                        tab.appendLine(); // 빈 줄 추가
                        tab.appendLine(); // 빈 줄 추가
                        String str = (String) aspects[0]; // 천문학적 관계 데이터의 첫 번째 문자열 가져오기
                        if (dual_ring) { // 이중 링 모드인 경우
                            String title = ChartMode // 차트 모드가 비교 모드인지 확인하여 제목 설정
                                    .isAstroMode(ChartMode.COMPARISON_MODE)
                                            ? now
                                                    .getName()
                                            : ChartMode.getModeName(true,
                                                    true);
                            int num_col = str.substring(str.indexOf("$") + 1) // 열 수 계산
                                    .indexOf("$");
                            int gap = num_col - 2 * title.length(); // 제목과 열 간의 간격 계산
                            title = Resource.spacePreFilled("", gap / 2) // 제목을 중앙에 배치
                                    + title;
                            tab.appendLine(title); // 제목 추가
                        }
                        StringTokenizer st = new StringTokenizer(str, "$"); // 천문학적 관계 데이터를 토큰화
                        int num_row = st.countTokens(); // 토큰의 개수를 세어 행 수를 계산
                        String title = ChartMode // 차트 모드가 비교 모드인지 확인하여 제목 설정
                                .isAstroMode(ChartMode.COMPARISON_MODE)
                                        ? birth
                                                .getName()
                                        : Resource.getString("birth_chart");
                        int pos = -(num_row - title.length() - 2) / 2 - 2; // 제목의 위치를 계산
                        while (st.hasMoreTokens()) { // 토큰이 남아있는 동안 반복
                            str = st.nextToken(); // 다음 토큰 가져오기
                            if (dual_ring) { // 이중 링 모드인 경우
                                if (pos < 0 || pos >= title.length()) { // 제목의 범위를 벗어난 경우
                                    str = "  " + str; // 공백 추가
                                } else {
                                    String s = title.substring(pos, pos + 1); // 제목에서 현재 위치의 문자 가져오기
                                    if (FileIO.isAsciiString(s, true)) // ASCII 문자열인지 확인
                                        s += " "; // ASCII 문자열인 경우 공백 추가
                                    str = s + str; // 제목 문자와 토큰 결합
                                }
                                pos++; // 위치 증가
                            }
                            tab.appendLine(str); // 문자열을 탭에 줄바꿈과 함께 추가
                        }
                    }
                }
                now_sign_tip = sign_tip; // 현재 별자리 팁을 업데이트
            }
            if (!show_horiz && show_fixstar) { // 수평선 표시가 비활성화되고 고정 별자리를 표시하는 경우
                boolean has_fixstar = false; // 고정 별자리 존재 여부 초기화
                for (int i = 0; i < fixstar_names.length; i++) { // 고정 별자리 배열을 순회
                    if (!Calculate.isValid(fixstar_sign_pos[i])) // 고정 별자리 위치가 유효하지 않은 경우 건너뜀
                        continue;
                    String e_str = (explain_star && Resource // 고정 별자리 설명이 활성화된 경우
                            .hasKey(fixstar_signs[i] + explain_fixstar)) ? Resource
                                    .getString(fixstar_signs[i] + explain_fixstar) : ""; // 설명 문자열 가져오기
                    tip.addTip(FIXSTAR_OFFSET + i, true, fixstar_signs[i] // 팁에 고정 별자리 정보 추가
                            + ": " + e_str);
                    if (!Calculate.isValid(fixstar_sign_azimuth[i])) // 고정 별자리 방위각이 유효하지 않은 경우 건너뜀
                        continue;
                    has_fixstar = true; // 고정 별자리가 있음을 표시
                    boolean astrolog_coord = Resource // 천문 좌표 설정 여부 확인
                            .getPrefInt("astrolog_coord") != 0;
                    String prefix = fixstar_signs[i]; // 고정 별자리 이름 가져오기
                    if (table != null) { // 테이블이 null이 아닌 경우
                        prefix += "     : "; // 접두사에 공백과 콜론 추가
                    } else {
                        prefix += ": "; // 접두사에 콜론만 추가
                    }
                    String str = prefix
                            + cal.formatDegree(fixstar_sign_pos[i], // 고정 별자리 위치를 포맷팅
                                    stellar_sign_pos, full_stellar_signs, "  ",
                                    false, true)
                            + "   "
                            + birth_sign_status_fill.replaceAll("@", "  "); // 별자리 상태 문자열에서 '@'를 공백으로 대체
                    tab.append(str); // 문자열을 탭에 추가
                    str = prefix
                            + cal.formatDegree(fixstar_sign_azimuth[i], "  ", // 고정 별자리 방위각을 포맷팅
                                    null, null,
                                    ChartMode.mountainBased(degree_mode), // 산 기반 차트 모드 확인
                                    astrolog_coord);
                    if (Calculate.isValid(fixstar_sign_alt[i])) { // 고정 별자리 고도가 유효한 경우
                        str += " "
                                + FileIO.formatDouble(fixstar_sign_alt[i], 2, // 고도를 소수점 2자리로 포맷팅
                                        1, true, true);
                    }
                    if (!e_str.equals("")) { // e_str이 비어있지 않은 경우
                        e_str = "\n  "
                                + Resource.getSpaceFilled(fixstar_signs[i]) // 고정 별자리 이름에 공백 추가
                                + e_str;
                    }
                    tip.addTip(FIXSTAR_OFFSET + i, false, str + e_str); // 팁에 고정 별자리 정보 추가
                    tab.appendLine(str); // 문자열을 탭에 줄바꿈과 함께 추가
                    now_sign_tip += "\n" + str; // 현재 별자리 팁에 문자열 추가
                }
                if (has_fixstar) { // 고정 별자리가 있는 경우
                    tab.appendLine(); // 빈 줄 추가
                    showStarList(birth_table, fixstar_table, mountain_signs, // 고정 별자리 리스트 표시
                            null, explain_fixstar, false);
                }
            }
            if (birth_data || show_now) { // 출생 데이터 또는 현재 표시 여부 확인
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // ASTRO_MODE가 아닌 경우
                    tab.appendLine();
                    if (table != null) { // 별자리 데이터를 저장한 테이블이 null이 아닌 경우
                        String star_sign_key = Resource.getString("star_sign_key"); // 별자리 키 문자열 가져오기
                        for (int i = 0; i < signs.length; i++) { // 모든 별자리에 대해 반복
                            int order = sign_display_orders[i]; // 별자리 표시 순서 가져오기
                            if (!Calculate.isValid(sign_pos[order])) // 별자리 위치가 유효하지 않은 경우 건너뜀
                                continue;
                            LinkedList star_list = (LinkedList) table.get(signs[order] + star_sign_key); // 현재 별자리의 키에
                                                                                                         // 해당하는 리스트
                                                                                                         // 가져오기
                            if (star_list != null) { // 리스트가 null이 아닌 경우
                                int count = 0; // 별자리 값 카운터 초기화
                                tab.append(signs[order] + ": "); // 별자리 이름 추가
                                for (ListIterator iter = star_list.listIterator(); iter.hasNext();) { // 리스트의 각 값을 순회
                                    String val = (String) iter.next(); // 리스트의 현재 값 가져오기
                                    if (count++ == 0) // 첫 번째 값인 경우
                                        tab.append(val); // 값을 추가
                                    else
                                        tab.append(", " + val); // 이후 값은 쉼표로 구분하여 추가
                                }
                                tab.appendLine(); // 줄바꿈 추가
                            }
                        }
                    ...
                    tab.appendLine(); // 빈 줄 추가
                    tab.appendLine(Resource.getString("star_sign_key")); // 별자리 키 문자열 추가
                }
                if (birth_data && !ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // 출생 데이터가 있고 ASTRO_MODE가 아닌 경우
                    day_table = getStarSigns(birth_poles, birth_sign_pos, true); // 출생 별자리 데이터 가져오기
                    computeEightCharData(); // 팔자 데이터 계산
                }
            }
            if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) { // ASTRO_MODE가 아닌 경우
                String[] seq = Resource.getStringArray("star_sign_seq"); // 별자리 시퀀스 문자열 배열 가져오기
                showStarList(table, master_table, seq, year_signs, explain, !birth_data); // 별자리 리스트 표시
            }
        }
    }
