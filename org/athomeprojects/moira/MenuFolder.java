...
public class MenuFolder {
    ...
    private void addPrefMenu(Menu menu)
    {
        ...
        String langTxt = "한글 표시 (&L)";
        String tmpTxt = "한글로 표시 하시겠습니까?"; 
        
        if(is_simplified) {
        	langTxt = "繁体中文 (須重新啟動)(&L)";
        	tmpTxt = "轉為繁体中文 (須重新啟動)?";
        }
//        lang.setText(Resource.getString("pref_lang_"
//                + (is_simplified ? "traditional" : "simplified")));
        lang.setText(langTxt);
        final String langDialogTxt = tmpTxt;
...
//                    return;
                ...
                if (!Message.question(langDialogTxt))
                    return;
                if (!Moira.getTable().checkForSave())
                    return;
